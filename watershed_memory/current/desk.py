...
def _has_extension(db, name):
    return db.execute("SELECT 1 FROM sqlite_master WHERE name GLOB ? LIMIT 1", (name + "_*",)).fetchone() is not None


def _dispatch(db, case_id):
    ...
    if not db.execute("SELECT 1 FROM dispatch_settings WHERE case_id=?", (case_id,)).fetchone():
        return None
    state, _, profile = _settings(db, case_id)
    ...
    allowance = AllowanceSnapshot(*(raw[key] for key in (
        "scripted_limit", "scripted_used", "provider_limit", "provider_used"
    )))
    remaining = (allowance.scripted_limit - allowance.scripted_used if profile.mode == "SCRIPTED_SDK"
                 else allowance.provider_limit - allowance.provider_used)
    return {
        "history_count": status.history_count, "suppressed_count": status.suppressed_count,
        "assessment_count": status.assessment_count,
        "active_attempt_id": status.active_attempt_id, "active_status": status.active_status,
        "health_evaluated_at": iso(status.health_evaluated_at),
        "numeric_event_id": status.numeric_event_id, "execution_mode": profile.mode,
...
def _work(db, case_id, now):
    ...
    for raw in db.execute(
        ...
        "WHERE r.case_id=? AND r.status IN " + rows.ACTIVE + " ORDER BY r.kind LIMIT 3", (case_id,),
    ).fetchall():
        review = rows.record(raw)
        actions = ["MODIFY", "DEFER", "DISMISS", "CANCEL"]
        if review.status in ("PROPOSED", "DEFERRED") and (
            review.next_check_at is None or timedelta(0) < review.next_check_at - now <= timedelta(days=30)
        ):
            actions.insert(0, "APPROVE")
        evidence = [row[0] for row in db.execute(
            "SELECT event_id FROM current_review_evidence WHERE case_id=? AND task_id=? "
            "ORDER BY link_id DESC LIMIT 3", (case_id, review.task_id),
        )]
        for identity in evidence:
            event_id(identity)
        work.append({**work_receipt(review), "kind": review.kind, "reason": review.reason,
                     "created_at": iso(review.created_at), "updated_at": iso(review.updated_at),
                     "evidence_event_ids": evidence, "available_actions": actions})
    return work
...
def _dimensions(work, health, baseline_assessed):
    ...
    coverage = ("MISSING" if set(health.policy.required_parameters) <= missing else
                "DEGRADED" if coverage_issue else "SUFFICIENT")
    observation_status = ("CHANGE_UNDER_REVIEW" if observation else
                          "MONITORING" if baseline_assessed else "BASELINE_PENDING")
    return [
        {"id": "physical", "label": "Watershed recovery", "status": "NOT_ASSESSED",
         "detail": "Awaiting a scoped recovery assessment."},
        {"id": "observation", "label": "Source-water review",
         "status": observation_status,
         "detail": ("An observation review is being followed." if observation else
                    "No observation review is currently open." if baseline_assessed else
                    "Awaiting the first source assessment.")},
        {"id": "field", "label": "Field work & verification", "status": "NOT_PLANNED",
         "detail": "No structured field work has been planned."},
        {"id": "coverage", "label": "Monitoring coverage",
         "status": coverage,
         "detail": "Some required readings are missing, stale or null." if coverage_issue
         else "Required station readings are available within the freshness policy."},
    ]


class CurrentDesk:
    def __init__(self, cases: CaseStore, case_id: str):
    ...
    def _read(self, *, now: datetime, task_id: str | None = None) -> tuple[dict, dict | None]:
        now = utc(now)
        with self.cases._connect() as db:
            ...
            source = {
                "source_id": health.source_id, "station_id": health.station_id,
                ...
                "null_parameters": list(health.null_parameters), "readings": [],
            }
            ...
            for reading in health.series:
                ...
                if reading.version_id is None:
                    ...
                    flags.append("NULL")
                if reading.observed_at is not None and now - reading.observed_at > timedelta(seconds=health.policy.freshness_seconds):
                    flags.append("STALE")
                source["readings"].append({
                    "parameter_code": reading.parameter_code, "label": LABELS[reading.parameter_code],
                    "unit": reading.unit, "value": None if reading.value is None else str(reading.value),
                    "observed_at": iso(reading.observed_at), "retrieved_at": iso(reading.retrieved_at),
                    "approval_status": reading.approval_status, "flags": flags,
                })
            latest = db.execute(
                "SELECT event_id FROM watch_events WHERE monitor_id=? "
                "ORDER BY interval_start DESC,revision DESC LIMIT 1", (config.monitor_id,),
            ).fetchone()
            interval = None
            if latest is not None:
                facts = _load_context(db, self.case_id, latest[0], evaluated_at=now).current
                interval = {"event_id": facts.event_id, "start": iso(facts.interval_start),
                            "end": iso(facts.interval_end), "coverage": facts.interval_coverage,
                            "missing_parameters": list(facts.missing_parameters)}
            work = _work(db, self.case_id, now)
            dispatch = _dispatch(db, self.case_id)
            assessments = []
            baseline_assessed = False
            if _has_extension(db, "delivery"):
                check_delivery_schema(db)
                baseline_assessed = db.execute(
                ...
                recent = db.execute(
                    ...
                    "ORDER BY evaluated_at DESC,attempt_id DESC LIMIT 10", (self.case_id,),
                ).fetchall()
                for row in recent:
            ...
            value = {"evaluated_at": now.isoformat(),
                    "case": {"case_id": self.case_id, "revision": case["revision"], "simulated": config.simulated},
                    "source": source, "interval": interval, "work": work, "dispatch": dispatch,
                    "dimensions": _dimensions(work, health, baseline_assessed), "assessments": assessments}
            current_work = None
    ...
    def respond(self, action: HumanAction, *, request_id: str, now: datetime) -> dict:
        ...
        return {"receipt": work_receipt(receipt), "snapshot": snapshot, "current_work": current_work}
    ...
    def assessment(self, attempt_id: str) -> dict:
        with self.cases._connect() as db:
            ...
            if _has_extension(db, "dispatch"):
                check_dispatch_schema(db)
                item = db.execute("SELECT attention_json FROM dispatch_attempts WHERE case_id=? AND attempt_id=?", (self.case_id, attempt_id)).fetchone()
                if item is not None:
                    attention = item[0]
